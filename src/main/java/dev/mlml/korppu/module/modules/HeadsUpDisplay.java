...
public class HeadsUpDisplay extends Module {
    ...
    public void onRender(DrawContext drawContext, float tickDelta) {
        ...
        for (Module m : modules) {
            ...
            String format = TextFormatter.format("%2%3[%4%s%3]%1 %5%s%s", TextFormatter.Code.RESET, TextFormatter.Code.BOLD, TextFormatter.Code.GRAY, TextFormatter.Code.WHITE, m.isEnabled() ? TextFormatter.Code.GREEN : TextFormatter.Code.RED, m.getKeybind()
                                                                                                                                                                                                                                                    .getBoundKeyLocalizedText()
                                                                                                                                                                                                                                                    .getString(), m.getName(), status.isEmpty() ? "" : " : " + status);
        ...
        renderText(drawContext, TextFormatter.format("[%2%3Korppu%1 v%2%3%s%1] %s FPS", TextFormatter.Code.RESET, TextFormatter.Code.BOLD, TextFormatter.Code.GRAY, KorppuMod.VERSION, KorppuMod.mc.getCurrentFps()), Corner.TOP_RIGHT, 0);
    }
    ...
    public String getStatus() {
        ...
        return TextFormatter.format("%1%s | %2%s", TextFormatter.Code.GREEN, enabled, TextFormatter.Code.RED, ModuleManager.getModules()
                                                                                                                           .size() - enabled);
    }
    ...
    private enum Corner {
        ...
        public int[] getBase(int textWidth, int textHeight, int row) {
            ...
            return switch (this) {
                ...
                case TOP_RIGHT -> new int[]{KorppuMod.mc.getWindow()
                                                        .getScaledWidth() - textWidth / 2 - gap, gap + (textHeight + gap) * row};
                case BOTTOM_LEFT -> new int[]{gap + textWidth / 2, KorppuMod.mc.getWindow()
                                                                               .getScaledHeight() + gap + (textHeight + gap) * row};
                case BOTTOM_RIGHT -> new int[]{KorppuMod.mc.getWindow()
                                                           .getScaledWidth() - textWidth / 2 - gap, KorppuMod.mc.getWindow()
                                                                                                                .getScaledHeight() + gap + (textHeight + gap) * row};
            };
        }
    }
