...
public class FastMine extends Module
{
    ...
    public FastMine()
    {
        super("FastMine", "Instantly mine blocks", ModuleType.WORLD, GLFW.GLFW_KEY_LEFT_BRACKET);

        FastMinePacketHandler packetHandler = new FastMinePacketHandler();
    ...
    public void onTick()
    {
        ...
        if (latest.add(0.5, 0.5, 0.5).distanceTo(p) >= KorppuMod.mc.interactionManager.getReachDistance())
        {
            posList.remove(0);
            return;
        }
    ...
    public class FastMinePacketHandler
    {
