...
public class InstaBow extends Module {
    ...
    public InstaBow() {
        super("InstaBow", "Shoot bows instantly", GLFW.GLFW_KEY_APOSTROPHE);

        KorppuMod.eventManager.register(this);
    }

    private float getYawDelta(LivingEntity target) {
        ...
        return MathHelper.wrapDegrees((float) (targetYaw - pYaw));
    }


    @Override
    public void onTick() {
        if (!aimAssist.getValue() || KorppuMod.mc.player == null || KorppuMod.mc.world == null) {
            return;
        }
        ...
        int pullback = bowItem.getMaxUseTime(KorppuMod.mc.player.getMainHandStack(), KorppuMod.mc.player) - KorppuMod.mc.player.getItemUseTimeLeft();
        ...
        for (Entity entity : KorppuMod.mc.world.getEntities()) {
            ...
            if (!KorppuMath.hasLOS(eyePosition, entity.getPos()
                                                      .add(0, h / 2f, 0), KorppuMod.mc.world, KorppuMod.mc.player)) {
                continue;
            }
            if (eyePosition.y < upper.y && eyePosition.y > origin.y) {
                if (bestTarget == null || Math.abs(getYawDelta(ent)) < Math.abs(getYawDelta(bestTarget))) {
                    bestTarget = ent;
                }
            }
        }
        ...
        Objects.requireNonNull(KorppuMod.mc.getNetworkHandler())
               .sendPacket(new PlayerMoveC2SPacket.LookAndOnGround(Rotations.getClientYaw(), Rotations.getClientPitch(), KorppuMod.mc.player.isOnGround()));
    ...
    public void onPacketSend(PacketEvent.Sent event) {
        ...
        if (event.getPacket() instanceof PlayerActionC2SPacket packet && packet.getAction() == PlayerActionC2SPacket.Action.RELEASE_USE_ITEM) {
            if (KorppuMod.mc.player == null || KorppuMod.mc.getNetworkHandler() == null || ModuleManager.doNotSendPackets()) {
                return;
            }

            Vec3d start = KorppuMod.mc.player.getPos().subtract(0, 1e-10, 0);
            Vec3d end = KorppuMod.mc.player.getPos().add(0, 1e-10, 0);
            KorppuMod.mc.getNetworkHandler()
                        .sendPacket(new ClientCommandC2SPacket(KorppuMod.mc.player, ClientCommandC2SPacket.Mode.START_SPRINTING));
